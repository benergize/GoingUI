function GoingUI(viewElement = -1,animate=true) {
	...
	this.bind = function(element = -1) {
		...
		element.forEach(el=>{
			...
			if(!thisUI.isset(el.dataset.gobound)) {
				...
				el.addEventListener("keypress", ev=>{ thisUI.set(name,el.type == "checkbox" ? el.checked : el.value,'input'); });
				el.addEventListener("keyup", ev=>{thisUI.set(name,el.type == "checkbox" ? el.checked : el.value, 'input'); });
				el.addEventListener("change", ev=>{  thisUI.set(name,el.type == "checkbox" ? el.checked : el.value,'input'); });
						...
						for(let attribute in attrs) {
							...
							if(attribute.indexOf(".") === -1 && String(dataFrom).indexOf(".") === -1) {
								...
								if(typeof attrs[attribute] === "object") {
									...
									if(typeof thisUI.data[dataFrom] == "string" || typeof thisUI.data[dataFrom] == "number") {
										el[attribute] = thisUI.data[dataFrom];
									}
									else if(typeof thisUI.data[dataFrom] == "object") {
										for(let z in thisUI.data[dataFrom]) {

											el[attribute][z] = thisUI.data[dataFrom][z];
										}
									}
								}
							}
	...
	this.setViewElement = function(vE,anim=this.animate) {
		...
		if(document.querySelectorAll(vE).length > 0) {
			...
			window.addEventListener("load",  ev=>{ thisUI.changeView(); });
		}
	}
